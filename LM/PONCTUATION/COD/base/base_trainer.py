...
class BaseTrainer:
    def __init__(
        ...
        # save processor
        self.tokenizer.save_pretrained(config["huggingface"]["args"]["local_dir"])
    ...
    def _is_best_epoch(self, score_punct, score_capit, save_max_metric_score=True) -> bool:
        """
        Check if the current model got the best composite metric score
        """
        # Define weights for punctuation and capitalization scores
        weight_punct = 1.0  # Weight for punctuation accuracy
        weight_capit = 0.5  # Weight for capitalization accuracy (penalized)

        # Calculate composite score with weighted average
        composite_score = (weight_punct * score_punct + weight_capit * score_capit) / (weight_punct + weight_capit)
        
        if save_max_metric_score and composite_score >= self.best_score:
            self.best_score = composite_score
            return True
        elif not save_max_metric_score and composite_score <= self.best_score:
            self.best_score = composite_score
            return True
        else:
            return False
